...
t.getNode(2).bootAtTime(100002);
t.getNode(3).bootAtTime(100003);
t.getNode(4).bootAtTime(100004);
t.getNode(5).bootAtTime(100005);
t.getNode(6).bootAtTime(100006);
t.getNode(7).bootAtTime(100007);
t.getNode(8).bootAtTime(100008);
...
for i in range(Max_Node):
    print ('Node %d : Total Send = %d Total Succ = %d Packet Loss Rate = %.2f%s Average Delay = %.2f ms' %
           (i+1,Send[i],Send_Succ[i],(1 - 1.0*Send_Succ[i] / Send[i]) * 100,'%', Delay_Sum[i] / Send_Succ[i]))
    tot_send += Send[i]
